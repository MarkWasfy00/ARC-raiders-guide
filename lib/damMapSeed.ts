...
import damData from './data/dam.json';
...
export async function seedDamMap() {
  console.log('🗺️  Starting Dam map markers seed...');

  try {
    // First, delete existing Dam markers
    const deleteResult = await prisma.mapMarker.deleteMany({
      where: {
        mapID: 'dam',
      },
    });
    console.log(`🗑️  Deleted ${deleteResult.count} existing Dam markers`);

    // Prepare markers for insertion
    const markers = (damData.allData as DamMarker[]).map((marker) => ({
      id: marker.id,
      lat: marker.lat,
      lng: marker.lng,
    ...
    console.log(`✅ Successfully seeded ${insertedCount} Dam map markers!`);
    ...
    console.error('❌ Error seeding Dam map markers:', error);
    throw error;
  }
}
